...
export async function startTestServer({ mongoUri, envOverrides = {} } = {}) {
  ...
  try {
    ...
    // Auto-planning (server/src/pipeline/recoveryPlan.js) is ON by default in a real
    // deployment, but the bulk of the suite exercises the DETECT -> EVALUATE stages one step at
    // a time and asserts the intermediate case states. Forcing it off here keeps those tests
    // deterministic; tests/recoveryPlan.test.js opts back in via envOverrides.
    process.env.RECOVERY_AUTOPLAN_ENABLED = "false";
