    ...
    <div className="flex items-center gap-2">
      {STEPS.map((label, i) => (
        <div key={label} className="flex flex-1 flex-col gap-1.5">
          <div className={`h-1 rounded-full ${i <= current ? "bg-brand-600" : "bg-slate-200"}`} />
          <span className={`hidden text-[11px] font-medium sm:block ${i <= current ? "text-brand-700" : "text-slate-400"}`}>{label}</span>
        </div>
      ))}
    </div>
  );
      ...
      <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white p-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-slate-400">Payment links, webhooks, Test Mode</div>
          </div>
        </div>
  ...
  return (
    <div className="space-y-5">
      <label className="block text-sm text-slate-600">
        ...
        />
        <span className="text-xs text-slate-400">{form.maxAttempts} attempt{form.maxAttempts === 1 ? "" : "s"}</span>
      </label>
      <label className="block text-sm text-slate-600">
        ...
        <input
          ...
          className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm text-brand-900 shadow-sm focus:border-brand-400 focus:outline-none focus:ring-1 focus:ring-brand-400"
        />
        <span className="mt-1 block text-xs text-slate-400">Cases above this amount are escalated for human review, never auto-recovered.</span>
      </label>
    </div>
  );
}
...
function ReadyStep() {
  return (
    <div className="flex flex-col items-center gap-4 py-4 text-center">
      ...
      <Link to="/demo" className="mt-2 rounded-xl bg-brand-700 px-6 py-3 text-sm font-semibold text-white shadow-sm hover:bg-brand-800">
        View the live demo
      </Link>
    </div>
  );
...
export default function Onboarding() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-canvas px-6 py-16">
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <FloatingCurrency symbol="₹" size="text-6xl" tone="text-brand-100" style={{ top: "8%", right: "10%" }} depth={10} />
        <FloatingCurrency symbol="$" size="text-2xl" tone="text-mint-300" style={{ bottom: "10%", left: "8%" }} depth={16} floatDelay={0.6} />
      </div>
      ...
      <div className="relative mx-auto max-w-lg">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-xs uppercase tracking-widest text-slate-400">Welcome to PayRevive</p>
        </div>

        <RevealOnScroll className="rounded-3xl border border-slate-200/80 bg-white p-8 shadow-card-hover">
