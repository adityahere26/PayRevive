        ...
        <form onSubmit={handleRun} className="flex flex-wrap items-end gap-3">
          <label className="text-sm text-slate-600">
            ...
            <input
              ...
              className="mt-1 block w-36 rounded-lg border border-slate-200 px-3 py-1.5 text-sm text-brand-900 shadow-sm focus:border-brand-400 focus:outline-none focus:ring-1 focus:ring-brand-400"
            />
          </label>
      ...
      {metrics && (
        <>
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            ...
            <StatTile label="Recovered Revenue" value={formatINR(metrics.recoveredRevenue)} hint={`${metrics.recoveredCases} recovered`} accent="emerald" />
            <StatTile label="Recovery Rate" value={`${Math.round(metrics.recoveryRate * 100)}%`} hint="of eligible revenue" accent="brand" />
            <StatTile label="Policy-Blocked / High-Value" value={metrics.escalatedCases} hint={`${formatINR(metrics.escalatedRevenue)} escalated for review`} accent="amber" />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-5">
            ...
            <StatTile label="Stopped / Rejected" value={metrics.stoppedCases} hint={formatINR(metrics.stoppedRevenue)} accent="slate" />
            <StatTile label="Window Expired" value={metrics.expiredCases} hint={formatINR(metrics.expiredRevenue)} accent="slate" />
            <StatTile
              label="Policy Violations"
              value={metrics.policyViolations}
              hint={metrics.policyViolations === 0 ? "0 — as expected" : "unexpected — investigate"}
              accent={metrics.policyViolations === 0 ? "emerald" : "red"}
            />
          </div>
            ...
            action={
              <label className="text-xs text-slate-500">
                ...
                <select
                  ...
                  className="ml-2 rounded-lg border border-slate-200 px-2 py-1 text-xs shadow-sm focus:border-brand-400 focus:outline-none focus:ring-1 focus:ring-brand-400"
                >
              ...
              <table className="w-full min-w-[760px] text-left text-sm">
                <thead className="sticky top-0 bg-white">
                  <tr className="border-b border-slate-100 text-xs uppercase tracking-wide text-slate-400">
                    <th className="px-6 pb-2 font-medium">Case</th>
                    <th className="px-6 pb-2 font-medium">Amount</th>
                    <th className="px-6 pb-2 font-medium">Root Cause</th>
                    <th className="px-6 pb-2 font-medium">Probability</th>
                    <th className="px-6 pb-2 font-medium">Intervention</th>
                    <th className="px-6 pb-2 font-medium">Policy Decision</th>
                    <th className="px-6 pb-2 font-medium">Outcome</th>
                    <th className="px-6 pb-2 font-medium">Recovered</th>
                  </tr>
                </thead>
                <tbody>
                  {visibleCases.map((c) => (
                    <tr key={c.index} className="border-b border-slate-50 transition-colors last:border-0 hover:bg-mint-50/60">
                      ...
                      <td className="px-6 py-2.5 font-semibold text-brand-900">{formatINR(c.amount)}</td>
                      <td className="px-6 py-2.5 text-slate-500">{humanize(c.rootCause) || "—"}</td>
                      <td className="px-6 py-2.5 text-slate-500">
                        {c.recoveryProbability != null ? `${Math.round(c.recoveryProbability * 100)}%` : "—"}
                      </td>
                      <td className="px-6 py-2.5 text-slate-500">{humanize(c.selectedIntervention) || "—"}</td>
                      <td className="px-6 py-2.5 text-slate-500">{humanize(c.policyDecision) || "—"}</td>
                      <td className="px-6 py-2.5">
                        <StatusBadge status={c.status} />
                      </td>
                      <td className="px-6 py-2.5 text-slate-500">{c.recoveredAmount > 0 ? formatINR(c.recoveredAmount) : "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
