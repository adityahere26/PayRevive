...
// full-bleed split composition, not a centered card floating on a plain canvas. The left
// panel is one of the product's few intentional bounded brand-gradient accents (the right
// panel — the actual page content — is fully light); it carries an editorial statement +
// floating currency circles. No marketing header/footer — the auth flow is deliberately
...
export function AuthShell({ eyebrow, title, subtitle, statement, statementSupport, children, footer }) {
  return (
    <div className="relative grid min-h-screen overflow-hidden bg-canvas lg:grid-cols-2">
      <div className="gradient-brand relative hidden overflow-hidden px-12 py-14 lg:flex lg:flex-col lg:justify-between">
        <div className="pointer-events-none absolute inset-0 overflow-hidden">
          <FloatingCurrency symbol="₹" size="text-8xl" tone="text-white/5" style={{ top: "-6%", right: "-4%" }} depth={8} />
          <FloatingCurrency symbol="₹" size="text-2xl" tone="border-accent-light/40 text-accent-light" style={{ top: "22%", left: "12%" }} depth={16} circle />
        ...
        <div className="relative max-w-sm">
          <p className="label-mono text-[11px] text-accent-light">Revenue recovery</p>
